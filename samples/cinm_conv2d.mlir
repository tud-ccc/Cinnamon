...
func.func @conv(%img : tensor<1x128x128x3xi16>, %flt : tensor<3x3x3x8xi16>, %bias:  tensor<1x126x126x8xi16>) {
    ...
    %rbuf = cinm.compute(%arg2 = %im2col: tensor<15876x27xi16>) -> tensor<15876x8xi16> {
        %flt = arith.constant <"..."> : tensor<27x8xi16>
        %conv = cinm.op.gemm %arg2, %flt : (tensor<15876x27xi16>, tensor<27x8xi16>) -> tensor<15876x8xi16>
