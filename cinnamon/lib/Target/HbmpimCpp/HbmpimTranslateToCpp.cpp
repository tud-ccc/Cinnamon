...
#define DEBUG_TYPE "translate-to-hbmpim-cpp"
...
static std::string getBool(bool in){
  if (in) return "true";
  else return "false";
}
...
static LogicalResult printOperation(CppEmitter &emitter, arith::DivUIOp divOp) {
...
static LogicalResult printOperation(CppEmitter &emitter,
                                    hbmpim::ReturnOp returnOp) {
  return success();
}



static LogicalResult printOperation(CppEmitter &emitter, hbmpim::SetDeviceConfigOp op) {
  raw_ostream &os = emitter.ostream();
  hbmpim::DeviceConfigurationType config = op.getResult().getType();
  os << "SetDeviceConfig(" << config.getNumBanks() << ", " << 
    config.getNumChannels() << ", "  <<
    config.getNumRanks() << ", " << config.getNumGrf() << ")"; 
  return success();
}
...
static LogicalResult printOperation(CppEmitter &emitter, hbmpim::ChangePIMModeOp op) {
  raw_ostream &os = emitter.ostream();
  os << "changePIMMode(dramMode::" << hbmpim::stringifyDRAMMode(op.getCurMode()) << 
    ", dramMode::" << hbmpim::stringifyDRAMMode(op.getNextMode()) << ")";
  return success();
}

static LogicalResult printOperation(CppEmitter &emitter, hbmpim::ProgramCrfOp op) {
  raw_ostream &os = emitter.ostream();
  os << "programCrf(" << emitter.getOrCreateName(op.getPimCmds()) << ")";
  return success();
}
...
static LogicalResult printOperation(CppEmitter &emitter, hbmpim::GetPimCmdsOp op) {
  if (failed(emitter.emitAssignPrefix(*op.getOperation())))
    return failure();
  raw_ostream &os = emitter.ostream();
  os << "PIMCmdGen::getPIMCmds(" ;
  if(op.getKerType() == hbmpim::PimKernelType::ADD) os << "KernelType::ADD";
  else if(op.getKerType() == hbmpim::PimKernelType::GEMV) os << "KernelType::GEMV";
  else if(op.getKerType() == hbmpim::PimKernelType::MUL) os << "KernelType::MUL";
  else if(op.getKerType() == hbmpim::PimKernelType::RELU) os << "KernelType::RELU";
  os << ", ";
  os << emitter.getOrCreateName(op.getJumpTake());
  os << ", ";
  os << emitter.getOrCreateName(op.getOddJump());
  os << ", ";
  os << emitter.getOrCreateName(op.getEvenJump());
  os << ")";
  return success();
}
...
static void printCompilationVar(hbmpim::HbmpimFuncOp &kernel, raw_ostream &os) {
  os << "COMPILE_" << kernel.getSymName();
}

static LogicalResult printOperation(CppEmitter &emitter,
                                    hbmpim::HbmpimModuleOp moduleOp) {
  CppEmitter::Scope scope(emitter);

  llvm::SmallVector<hbmpim::HbmpimFuncOp> kernels;
  for (Operation &op : moduleOp) {
    if (llvm::isa<hbmpim::HbmpimFuncOp>(op)) {
      kernels.push_back(llvm::cast<hbmpim::HbmpimFuncOp>(op));
    }
  }
  ...
  LogicalResult status =
      llvm::TypeSwitch<Operation *, LogicalResult>(&op)
          ...
          .Case<func::CallOp, func::ConstantOp, func::FuncOp,
                hbmpim::HbmpimFuncOp, func::ReturnOp, hbmpim::ReturnOp>(
              [&](auto op) { return printOperation(*this, op); })
          // SCF ops.
...
LogicalResult CppEmitter::emitType(Location loc, Type type) {
  ...
  if (auto pimVecType = dyn_cast<hbmpim::PIMCMDVecType>(type)){
    os << "vector<PIMCmd>";
    return success();
  }
...
LogicalResult hbmpim_emitc::HbmpimTranslateToCpp(Operation *op, raw_ostream &os) {
  bool declareVariablesAtTop = false;
  CppEmitter emitter(os, declareVariablesAtTop);
  LogicalResult res = success();
  op->walk<WalkOrder::PreOrder>([&](Operation *child) {
    if (llvm::isa<hbmpim::HbmpimModuleOp>(child)) {
