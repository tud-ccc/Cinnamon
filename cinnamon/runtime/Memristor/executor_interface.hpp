...
namespace memristor_runtime {
  ...
  struct memref_descriptor {
    ...
    operator memref_descriptor<D>() {
      ...
      for (size_t i = 0; i < D; i++) {
        result.sizes[i] = i < dimension_padding ? 1 : sizes[i - dimension_padding];
        result.strides[i] = i < dimension_padding ? 1 : strides[i - dimension_padding];
      }
      return result;
    }
    ...
    template<typename T>
    void print_dimension(size_t dimension, size_t start) {
  ...
  template<typename T>
  void memristor_gemm(int32_t crossbar_id, memref_descriptor<2>, memref_descriptor<2>);

  template<typename T>
  void memristor_gemv(int32_t crossbar_id, memref_descriptor<1>, memref_descriptor<1>);
