...
module {
   ...
   btfl.block @softmax_initial outs(%O: <1048576xf32, host>) platform #upmem {

    // %max = max(%A)
    %max = empty_buf() : <f32>
    fill_buf %max, 0xFF800000 : f32 : <f32>
    schedule<(red N)=(1) to (1048576)> ins(%a = %O: <(N), f32, ?> to host) outs(%max1 = %max : <f32, ?> to ?) {
    ...
    // %O = %A - splat(%max)
    schedule<(par M)=(1) to (1048576)> ins(%m = %max: <f32, ?> to ?) outs(%o = %O : <(M), f32, ?> to host) {
    ...
    // %O = exp(%O)
    schedule<(par M)=(1) to (1048576)> outs(%o = %O : <(M), f32, ?> to host) {
    ...
    schedule<(red N)=(1) to (1048576)> ins(%o = %O: <(N), f32, ?> to host) outs(%sum2 = %sum: <f32, ?> to ?) {
    ...
    // %O = %O / splat(%sum)
    schedule<(par M)=(1) to (1048576)> ins(%sum2 = %sum: <f32, ?> to ?) outs(%o = %O : <(M), f32, ?> to host) {
