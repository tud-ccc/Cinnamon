...
module {
  btfl.block @softmax_initial outs(%arg0 : <1048576xf32, host>)  platform #upmem {
    ...
    %buf_0 = empty_buf() : <(R * D), f32>
    fill_buf %buf_0, 0xFF800000 : f32 : <(R * D), f32>
    schedule<(par L, red N) = (1, 1) to (R * D, 1048576 | (R * D))>
            ins(%buf_3 = %arg0 : <(L * N), f32> to host)
            outs(%buf_4 = %buf_0 : <(L), f32>) {
    ...
    schedule<() = () to ()>
            ins(%buf_3 = %buf_0 : <(R * D), f32>)
            outs(%buf_4 = %buf : <f32>) {
      tile reduction factor symbolic<R * D> ins(%tile = %buf_3 sdim 0 : <(R * D), f32>) {
    ...
    %buf_2 = empty_buf() : <(R * D), f32>
    fill_buf %buf_2, 0.000000e+00 : f32 : <(R * D), f32>
    schedule<(par L, red M) = (1, 1) to (R * D, 1048576 | (R * D))>
            ins(%buf_3 = %buf : <f32>)
            outs(%buf_4 = %arg0 : <(L * M), f32> to host, %buf_5 = %buf_2 : <(L), f32>) {
