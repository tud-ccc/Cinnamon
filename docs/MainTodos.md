...
- [x] scatter/gather cost: keep the existing C++ logic for `upmem.scatter`/`upmem.gather` but replace the current latency estimates with numbers derived from real hardware measurements.
