...
- DPU kernel estimator (`upmem.wait_for`): integrate Hamid's cycle-accurate DPU simulator. Options in order of preference:
  - bring it in as an external C++ or Rust library linked into cinm-mlir
...
- Maybe add an exhaustive search option to compare bayesian solutions against the true best solutions for experiments
...
- Add some evaluation of the surrogate and make sure its learning parameters are adapted for the task
