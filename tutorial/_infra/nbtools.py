...
#
# ``build/bin`` comes first on purpose: build-cinnamon.sh links the LLVM tools
# (mlir-opt, mlir-translate, opt, llc, ...) of the LLVM it built against next to
# cinm-opt there, so those are guaranteed to match. The pixi environment also
# puts that directory on PATH.
_CINNAMON_BUILD_DIR = Path(
    os.environ.get("CINNAMON_BUILD_DIR") or _REPO_ROOT / "build"
)
_KNOWN_BIN_SUBDIRS: Sequence[Path] = (
    _CINNAMON_BUILD_DIR / "bin",
    # Where build-llvm.sh unpacks the prebuilt LLVM, and where it builds one
    _REPO_ROOT / "third-party" / "llvm-prebuilt" / "bin",
...
def _env_flags(var: str) -> List[str]:
    """Flags the environment sets for the compiler, e.g. ``CFLAGS``.

    The pixi environment compiles against its own headers and libraries rather
    than the system's, and says so through CFLAGS and LDFLAGS. A compiler
    invoked without them finds the wrong ones or none at all.
    """
...
def clang_path() -> Path:
    # Prefer the compiler the environment was set up with. conda-forge's clang
    # is not on PATH under the bare name -- the pixi environment has clang-22
    # and x86_64-conda-linux-gnu-clang, and exports CC pointing at the latter --
    # so looking for "clang" alone would silently fall through to the system
    # one, which is not the compiler the rest of the project uses.
    ...
    if not p:
        raise FileNotFoundError(
            "clang not found. Enter the pixi environment (`pixi shell`), set CC, "
            "or put a clang on PATH."
        )
    return p
...
def compile_ll_to_obj(ll_file: Path, obj_file: Optional[Path] = None, opt_level: str = "2") -> Path:
    ...
    cmd = [clang_path(), f"-O{opt_level}", *_env_flags("CFLAGS"), *os_cflags,
           "-c", str(ll_file), "-o", str(obj_file)]
    run(cmd)
    return obj_file
...
def compile_c_to_obj(c_file: Path, obj_file: Optional[Path] = None,
    ...
    cmd = [clang_path(), f"-O{opt_level}", "-std=c11", *_env_flags("CFLAGS"), *os_cflags,
           *extra_cflags, "-c", str(c_file), "-o", str(obj_file)]
    run(cmd)
    return obj_file
...
def link_executable(objs: Sequence[Path], exe_path: Path, extra_ldflags: Sequence[str] = ()) -> Path:
    ...
    cmd = [clang_path(), *[str(Path(o)) for o in objs], "-o", str(exe_path),
           *_env_flags("LDFLAGS"), *os_ldflags, *extra_ldflags]
    run(cmd)
    return exe_path
...
def compile_ll_to_obj_with_flags(ll_file: Path, obj_file: Optional[Path] = None,
    ...
    obj_file = Path(obj_file).resolve()
    os_cflags, _ = _platform_build_flags()
    cmd = [clang_path(), f"-O{opt_level}", *_env_flags("CFLAGS"), *os_cflags, *extra_cflags,
           "-c", str(ll_file), "-o", str(obj_file)]
    run(cmd)
    return obj_file
...
def compile_c_to_obj_with_flags(c_file: Path, obj_file: Optional[Path] = None,
    ...
    obj_file = Path(obj_file).resolve()
    os_cflags, _ = _platform_build_flags()
    cmd = [clang_path(), f"-O{opt_level}", f"-std={c_standard}", *_env_flags("CFLAGS"), *os_cflags,
           *extra_cflags, "-c", str(c_file), "-o", str(obj_file)]
    run(cmd)
    return obj_file
...
def link_executable_with_flags(objects: Sequence[Path], exe_path: Path,
    ...
    exe_path = Path(exe_path).resolve()
    _, os_ldflags = _platform_build_flags()
    cmd = [clang_path(), *[str(Path(o)) for o in objects], "-o", str(exe_path),
           *_env_flags("LDFLAGS"), *os_ldflags, *extra_ldflags]
    run(cmd)
    return exe_path
