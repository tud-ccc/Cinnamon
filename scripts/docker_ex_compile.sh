...
docker run --rm   -u "$(id -u):$(id -g)"   -v "$repo_root":/workspace   -w /workspace/runtime/Alpine   "$docker_image" /bin/sh -eu -c '
    ...
    aarch64-linux-gnu-g++ $CXXFLAGS -c aimc_state.cc -o aimc_state.o
    ...
    aarch64-linux-gnu-ld -static -nostdlib -no-pie --no-dynamic-linker       -e _start --defsym __dso_handle=0       -o /workspace/third-party/ALPINE/working_test/aimc_probe.out       example.o crt0.o alpine_runtime.o aimc_state.o runtime_shims.o memref_rt_min.o
  '
