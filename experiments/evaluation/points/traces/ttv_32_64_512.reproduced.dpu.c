// DPU kernel ATiM's UPMEM backend generates for ttv_32_64_512.reproduced.tir.py.
// Regenerate with `python points/dump_atim_c.py --only ttv_32_64_512.reproduced`.
...
int main() {
  ...
  for (int32_t k_1_0 = 0; k_1_0 < 4; ++k_1_0) {
    for (int32_t ax1 = 0; ax1 < 2; ++ax1) {
      ...
      mram_read((__mram_ptr void*)(A + (((tasklet_id * 128) + (k_1_0 * 32)) + cse_var_1)), A_local + cse_var_1, 64);
    }
    mram_read((__mram_ptr void*)(B + (k_1_0 * 16)), B_local + 0, 64);
    for (int32_t j_4 = 0; j_4 < 2; ++j_4) {
      for (int32_t k_1_2 = 0; k_1_2 < 16; ++k_1_2) {
        C_rf_global_local[j_4] = (C_rf_global_local[j_4] + (A_local[((j_4 * 16) + k_1_2)] * B_local[k_1_2]));
      }
    }
  }
