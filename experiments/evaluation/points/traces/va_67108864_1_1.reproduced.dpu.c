// DPU kernel ATiM's UPMEM backend generates for va_67108864_1_1.reproduced.tir.py.
// Regenerate with `python points/dump_atim_c.py --only va_67108864_1_1.reproduced`.
...
int main() {
  ...
  for (int32_t i_2 = 0; i_2 < 32; ++i_2) {
    mram_read((__mram_ptr void*)(A + ((tasklet_id * 8192) + (i_2 * 256))), A_local + 0, 1024);
    mram_read((__mram_ptr void*)(B + ((tasklet_id * 8192) + (i_2 * 256))), B_local + 0, 1024);
    for (int32_t i_4 = 0; i_4 < 256; ++i_4) {
      A_local[i_4] = (A_local[i_4] + B_local[i_4]);
    }
    mram_write(A_local + 0, (__mram_ptr void*)(C + ((tasklet_id * 8192) + (i_2 * 256))), 1024);
  }
}
