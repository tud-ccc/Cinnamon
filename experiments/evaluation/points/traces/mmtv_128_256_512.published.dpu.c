// DPU kernel ATiM's UPMEM backend generates for mmtv_128_256_512.published.tir.py.
// Regenerate with `python points/dump_atim_c.py --only mmtv_128_256_512.published`.
...
int main() {
  ...
  for (int32_t k_0 = 0; k_0 < 8; ++k_0) {
    for (int32_t ax1 = 0; ax1 < 2; ++ax1) {
      ...
      mram_read((__mram_ptr void*)(A + (((tasklet_id * 1024) + (k_0 * 128)) + cse_var_1)), A_local + cse_var_1, 256);
    }
    ...
    for (int32_t j_3 = 0; j_3 < 2; ++j_3) {
      ...
      for (int32_t k_1 = 0; k_1 < 32; ++k_1) {
        ...
        for (int32_t k_2 = 0; k_2 < 2; ++k_2) {
          ...
          C_local[j_3] = (C_local[j_3] + (A_local[(((j_3 * 64) + cse_var_2) + k_2)] * B_local[(cse_var_2 + k_2)]));
        }
      }
    }
  }
