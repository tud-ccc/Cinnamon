// DPU kernel ATiM's UPMEM backend generates for ttv_128_256_512.reproduced.tir.py.
// Regenerate with `python points/dump_atim_c.py --only ttv_128_256_512.reproduced`.
...
int main() {
  ...
  for (int32_t k_0 = 0; k_0 < 16; ++k_0) {
    ...
    for (int32_t ax1 = 0; ax1 < 2; ++ax1) {
      ...
      mram_read((__mram_ptr void*)(A + (((tasklet_id * 1024) + (k_0 * 64)) + cse_var_1)), A_local + cse_var_1, 128);
    }
    mram_read((__mram_ptr void*)(B + (k_0 * 32)), B_local + 0, 128);
    #pragma clang loop unroll(full)
    for (int32_t k_1 = 0; k_1 < 16; ++k_1) {
      ...
      for (int32_t j_4 = 0; j_4 < 2; ++j_4) {
        ...
        for (int32_t k_2 = 0; k_2 < 2; ++k_2) {
          ...
          C_local[j_4] = (C_local[j_4] + (A_local[(((j_4 * 32) + cse_var_2) + k_2)] * B_local[(cse_var_2 + k_2)]));
        }
      }
    }
    mram_write(C_local + 0, (__mram_ptr void*)(C + (tasklet_id * 2)), 8);
  }
}
