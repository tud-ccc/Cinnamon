// DPU kernel ATiM's UPMEM backend generates for mmtv_128_256_512.reproduced.tir.py.
// Regenerate with `python points/dump_atim_c.py --only mmtv_128_256_512.reproduced`.
  ...
  for (int32_t k_0 = 0; k_0 < 8; ++k_0) {
    ...
    for (int32_t j_3 = 0; j_3 < 2; ++j_3) {
      for (int32_t k_2 = 0; k_2 < 64; ++k_2) {
        C_local[j_3] = (C_local[j_3] + (A_local[((j_3 * 64) + k_2)] * B_local[k_2]));
      }
    }
  }
