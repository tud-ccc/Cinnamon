# TIR those decisions produced, for ('red', 33554432, 1, 1) (reproduced).
...
class Module:
    ...
    def main(A: T.Buffer((33554432,), "int64"), B: T.Buffer((1,), "int64")):
        ...
        with T.block("root"):
            ...
            B_rf_global_rf_shared = T.alloc_buffer((8, 1024, 1), "int64", scope="shared")
            B_rf_global_rf_shared_local = T.alloc_buffer((8, 1024, 1), "int64", scope="local")
            ...
            for i_0 in T.thread_binding(1024, thread="blockIdx.x", annotations={"bank": T.int64(1), "pragma_auto_unroll_max_step": T.int64(64), "pragma_unroll_explicit": T.int64(0)}):
                for i_1_0 in T.thread_binding(8, thread="threadIdx.x"):
                    ...
                    for i_1_1_0, i_1_1_1 in T.grid(32, 4):
                        for ax0 in range(32):
                            with T.block("A_local"):
                                v0 = T.axis.spatial(33554432, i_0 * 32768 + i_1_0 * 4096 + i_1_1_0 * 128 + i_1_1_1 * 32 + ax0)
                                T.reads(A[v0])
                                T.writes(A_local[v0])
                                A_local[v0] = A[v0]
                        for i_1_1_2 in range(32):
                            with T.block("C_rf_rf_update"):
                                vi_0, vi_1_0 = T.axis.remap("SS", [i_0, i_1_0])
                                vi_1_1 = T.axis.reduce(4096, i_1_1_0 * 128 + i_1_1_1 * 32 + i_1_1_2)
                                T.reads(B_rf_global_rf_shared_local[vi_1_0, vi_0, 0], A_local[vi_0 * 32768 + (vi_1_0 * 4096 + vi_1_1)])
                                T.writes(B_rf_global_rf_shared_local[vi_1_0, vi_0, 0])
                                T.block_attr({"meta_schedule.meta_schedule_rfactor_producer_block": T.int64(1), "meta_schedule.tiling_structure": "SRRR"})
                                B_rf_global_rf_shared_local[vi_1_0, vi_0, 0] = B_rf_global_rf_shared_local[vi_1_0, vi_0, 0] + A_local[vi_0 * 32768 + (vi_1_0 * 4096 + vi_1_1)]
            ...
            with T.block("C_init"):
                ...
                B[0] = T.int64(0)
            for i_0 in T.serial(1024, annotations={"pragma_auto_unroll_max_step": T.int64(64), "pragma_unroll_explicit": T.int64(0)}):
