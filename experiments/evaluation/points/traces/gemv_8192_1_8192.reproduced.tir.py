# TIR those decisions produced, for ('gemv', 8192, 1, 8192) (reproduced).
            ...
            for k_0 in T.thread_binding(16, thread="blockIdx.x", annotations={"bank": T.int64(1), "pragma_auto_unroll_max_step": T.int64(64), "pragma_unroll_explicit": T.int64(0)}):
                for i_0 in T.thread_binding(128, thread="blockIdx.y", annotations={"bank": T.int64(1)}):
                    for i_1 in T.thread_binding(16, thread="threadIdx.x"):
                        for i_2 in range(1):
                            for i_3_init, i_4_init in T.grid(1, 4):
                                with T.block("C_rf_init"):
                                    v_i = T.axis.spatial(8192, i_0 * 64 + i_1 * 4 + i_2 * 4 + i_3_init * 4 + i_4_init)
                            ...
                            for k_1_0, i_3, k_1_1 in T.grid(16, 1, 2):
                                for ax0, ax1 in T.grid(4, 16):
                                    with T.block("A_local"):
                                        v0 = T.axis.spatial(8192, i_0 * 64 + i_1 * 4 + ax0)
                                        v1 = T.axis.spatial(8192, k_0 * 512 + k_1_0 * 32 + k_1_1 * 16 + ax1)
                                        T.reads(A[v0, v1])
                                ...
                                for ax0 in range(16):
                                    with T.block("B_local"):
                                        v0 = T.axis.spatial(8192, k_0 * 512 + k_1_0 * 32 + k_1_1 * 16 + ax0)
                                        T.reads(B[v0])
                                        T.writes(B_local[v0])
                                        T.block_attr({"meta_schedule.cooperative_fetch": 1})
                                        B_local[v0] = B[v0]
                                for i_4, k_1_2 in T.grid(4, 16):
                                    with T.block("C_rf_update"):
                                        v_i = T.axis.spatial(8192, i_0 * 64 + i_1 * 4 + i_2 * 4 + i_3 * 4 + i_4)
                                        vk_0 = T.axis.spatial(16, k_0)
                                        vk_1 = T.axis.reduce(512, k_1_0 * 32 + k_1_1 * 16 + k_1_2)
