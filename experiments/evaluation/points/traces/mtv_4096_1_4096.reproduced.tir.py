# TIR those decisions produced, for ('mtv', 4096, 1, 4096) (reproduced).
...
class Module:
    ...
    def main(A: T.Buffer((4096, 4096), "int32"), B: T.Buffer((4096,), "int32"), C: T.Buffer((4096,), "int32")):
        ...
        with T.block("root"):
                        ...
                        for i_2 in range(1):
                            for i_3_init, i_4_init in T.grid(1, 2):
                                with T.block("C_rf_init"):
                                    v_i = T.axis.spatial(4096, i_0 * 32 + i_1 * 2 + i_2 * 2 + i_3_init * 2 + i_4_init)
                            ...
                            for k_1_0 in range(16):
                                for ax0, ax1 in T.grid(2, 16):
                                    with T.block("A_local"):
                                        v0 = T.axis.spatial(4096, i_0 * 32 + i_1 * 2 + ax0)
                                ...
                                for ax0 in range(16):
                                    with T.block("B_local"):
                                        ...
                                        T.block_attr({"meta_schedule.cooperative_fetch": 1})
                                        B_local[v0] = B[v0]
                                for i_3, k_1_1, i_4, k_1_2 in T.grid(1, 4, 2, 4):
                                    with T.block("C_rf_update"):
                                        v_i = T.axis.spatial(4096, i_0 * 32 + i_1 * 2 + i_2 * 2 + i_3 * 2 + i_4)
                                        vk_0 = T.axis.spatial(16, k_0)
                                        vk_1 = T.axis.reduce(256, k_1_0 * 16 + k_1_1 * 4 + k_1_2)
            ...
            for i in T.serial(4096, annotations={"pragma_auto_unroll_max_step": T.int64(64), "pragma_unroll_explicit": T.int64(0)}):
                with T.block("C_init"):
