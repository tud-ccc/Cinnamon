// DPU kernel ATiM's UPMEM backend generates for ttv_512_512_512.published.tir.py.
// Regenerate with `python points/dump_atim_c.py --only ttv_512_512_512.published`.
...
int main() {
  ...
  for (int32_t j_4_init = 0; j_4_init < 8; ++j_4_init) {
    C_local[j_4_init] = 0;
  }
  for (int32_t k_0 = 0; k_0 < 32; ++k_0) {
    for (int32_t ax1 = 0; ax1 < 8; ++ax1) {
      int32_t cse_var_1 = (ax1 * 16);
      mram_read((__mram_ptr void*)(A + (((tasklet_id * 4096) + (k_0 * 128)) + cse_var_1)), A_local + cse_var_1, 64);
    }
    mram_read((__mram_ptr void*)(B + (k_0 * 16)), B_local + 0, 64);
    for (int32_t j_4 = 0; j_4 < 8; ++j_4) {
      for (int32_t k_2 = 0; k_2 < 16; ++k_2) {
        C_local[j_4] = (C_local[j_4] + (A_local[((j_4 * 16) + k_2)] * B_local[k_2]));
      }
    }
  }
