# TIR those decisions produced, for ('gemv', 8192, 1, 16384) (reproduced).
                        ...
                        for i_2 in range(1):
                            for i_3_init, i_4_init in T.grid(4, 1):
                                with T.block("C_rf_init"):
                                    v_i = T.axis.spatial(8192, i_0 * 64 + i_1 * 4 + i_2 * 4 + i_3_init + i_4_init)
                            ...
                            for k_1_0, i_3, k_1_1 in T.grid(64, 4, 1):
                                for ax0, ax1 in T.grid(1, 16):
                                    with T.block("A_local"):
                                        v0 = T.axis.spatial(8192, i_0 * 64 + i_1 * 4 + i_3 + ax0)
                                        v1 = T.axis.spatial(16384, k_0 * 1024 + k_1_0 * 16 + ax1)
                                        T.reads(A[v0, v1])
                                ...
                                for ax0 in range(16):
                                    with T.block("B_local"):
                                        v0 = T.axis.spatial(16384, k_0 * 1024 + k_1_0 * 16 + ax0)
                                        T.reads(B[v0])
                                        T.writes(B_local[v0])
                                        T.block_attr({"meta_schedule.cooperative_fetch": 1})
                                        B_local[v0] = B[v0]
                                for i_4, k_1_2 in T.grid(1, 16):
                                    with T.block("C_rf_update"):
                                        v_i = T.axis.spatial(8192, i_0 * 64 + i_1 * 4 + i_2 * 4 + i_3 + i_4)
                                        vk_0 = T.axis.spatial(16, k_0)
                                        vk_1 = T.axis.reduce(1024, k_1_0 * 16 + k_1_1 * 16 + k_1_2)
