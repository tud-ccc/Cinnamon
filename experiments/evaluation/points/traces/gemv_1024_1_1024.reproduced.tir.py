# TIR those decisions produced, for ('gemv', 1024, 1, 1024) (reproduced).
...
class Module:
    ...
    def main(A: T.Buffer((1024, 1024), "int32"), B: T.Buffer((1024,), "int32"), C: T.Buffer((1024,), "int32"), ALPHA: T.Buffer((1,), "int32")):
        ...
        with T.block("root"):
                            ...
                            for k_1_0 in range(8):
                                for ax0, ax1 in T.grid(2, 8):
                                    with T.block("A_local"):
                                        ...
                                        v1 = T.axis.spatial(1024, k_0 * 64 + k_1_0 * 8 + ax1)
                                        T.reads(A[v0, v1])
                                ...
                                for ax0 in range(8):
                                    with T.block("B_local"):
                                        v0 = T.axis.spatial(1024, k_0 * 64 + k_1_0 * 8 + ax0)
                                        T.reads(B[v0])
                                        T.writes(B_local[v0])
                                        T.block_attr({"meta_schedule.cooperative_fetch": 1})
                                        B_local[v0] = B[v0]
                                for i_3 in range(1):
                                    for k_1_1, i_4, k_1_2 in T.grid(8, 2, 1):
                                        with T.block("C_rf_update"):
                                            v_i = T.axis.spatial(1024, i_0 * 64 + i_1 * 4 + i_2 * 2 + i_3 * 2 + i_4)
                                            vk_0 = T.axis.spatial(16, k_0)
                                            vk_1 = T.axis.reduce(64, k_1_0 * 8 + k_1_1 + k_1_2)
            ...
            for i in T.serial(1024, annotations={"pragma_auto_unroll_max_step": T.int64(64), "pragma_unroll_explicit": T.int64(0)}):
                with T.block("C_init"):
