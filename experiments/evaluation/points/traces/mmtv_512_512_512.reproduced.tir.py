# TIR those decisions produced, for ('mmtv', 512, 512, 512) (reproduced).
                        ...
                        for i_1, i_2, j_2 in T.grid(1, 1, 1):
                            for i_3_init, j_3_init, i_4_init, j_4_init in T.grid(1, 4, 1, 2):
                                with T.block("C_init"):
                                    ...
                                    v_j = T.axis.spatial(512, j_0 * 128 + j_1 * 8 + j_2 * 8 + j_3_init * 2 + j_4_init)
                                    T.reads()
                                    T.writes(C_local[v_i, v_j])
                                    T.block_attr({"meta_schedule.tiling_structure": "SSSRSRSR"})
                                    C_local[v_i, v_j] = 0
                            for k_0, i_3, j_3, k_1 in T.grid(1, 1, 4, 16):
                                for ax0, ax1, ax2 in T.grid(1, 2, 32):
                                    with T.block("A_local"):
                                        v0 = T.axis.spatial(512, i_0 + ax0)
                                        v1 = T.axis.spatial(512, j_0 * 128 + j_1 * 8 + j_3 * 2 + ax1)
                                        v2 = T.axis.spatial(512, k_1 * 32 + ax2)
                                        T.reads(A[v0, v1, v2])
                                ...
                                for ax0, ax1 in T.grid(1, 32):
                                    with T.block("B_local"):
                                        v0 = T.axis.spatial(512, i_0 + ax0)
                                        v1 = T.axis.spatial(512, k_1 * 32 + ax1)
                                        T.reads(B[v0, v1])
                                ...
                                for i_4, j_4, k_2 in T.grid(1, 2, 32):
                                    with T.block("C_update"):
                                        ...
                                        v_j = T.axis.spatial(512, j_0 * 128 + j_1 * 8 + j_2 * 8 + j_3 * 2 + j_4)
                                        v_k = T.axis.reduce(512, k_0 * 512 + k_1 * 32 + k_2)
