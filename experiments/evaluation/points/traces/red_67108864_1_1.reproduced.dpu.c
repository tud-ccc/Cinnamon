// DPU kernel ATiM's UPMEM backend generates for red_67108864_1_1.reproduced.tir.py.
// Regenerate with `python points/dump_atim_c.py --only red_67108864_1_1.reproduced`.
...
int64_t B_rf_global_rf_shared[16];
int64_t red_buf0[16];
__mram_noinit int64_t A[65536];
...
int main() {
  ...
  int64_t* B_rf_global_rf_shared_local = (int64_t*) mem_alloc(1 * sizeof(int64_t));
    int64_t* A_local = (int64_t*) mem_alloc(32 * sizeof(int64_t));
    B_rf_global_rf_shared_local[0] = (int64_t)0;
  for (int32_t i_1_1_0 = 0; i_1_1_0 < 8; ++i_1_1_0) {
    for (int32_t i_1_1_1 = 0; i_1_1_1 < 16; ++i_1_1_1) {
      mram_read((__mram_ptr void*)(A + (((tasklet_id * 4096) + (i_1_1_0 * 512)) + (i_1_1_1 * 32))), A_local + 0, 256);
      for (int32_t i_1_1_2 = 0; i_1_1_2 < 32; ++i_1_1_2) {
        B_rf_global_rf_shared_local[0] = (B_rf_global_rf_shared_local[0] + A_local[i_1_1_2]);
      }
    }
  ...
  barrier_wait(&barrier);
  if (tasklet_id < 8) {
    handshake_wait_for((tasklet_id + 8));
