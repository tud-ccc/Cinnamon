// DPU kernel ATiM's UPMEM backend generates for geva_67108864_1_1.reproduced.tir.py.
// Regenerate with `python points/dump_atim_c.py --only geva_67108864_1_1.reproduced`.
...
int main() {
  ...
  for (int32_t i_2 = 0; i_2 < 16; ++i_2) {
    mram_read((__mram_ptr void*)(A + ((tasklet_id * 2048) + (i_2 * 128))), A_local + 0, 512);
    mram_read((__mram_ptr void*)(B + ((tasklet_id * 2048) + (i_2 * 128))), B_local + 0, 512);
    for (int32_t i_3 = 0; i_3 < 8; ++i_3) {
      #pragma clang loop unroll(full)
      for (int32_t i_4 = 0; i_4 < 16; ++i_4) {
        int32_t cse_var_1 = ((i_3 * 16) + i_4);
        C_local[i_4] = ((alpha_val * A_local[cse_var_1]) + (beta_val * B_local[cse_var_1]));
      }
      mram_write(C_local + 0, (__mram_ptr void*)(C + (((tasklet_id * 2048) + (i_2 * 128)) + (i_3 * 16))), 64);
    }
  }
}
