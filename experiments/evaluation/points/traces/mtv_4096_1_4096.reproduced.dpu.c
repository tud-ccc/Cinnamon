// DPU kernel ATiM's UPMEM backend generates for mtv_4096_1_4096.reproduced.tir.py.
// Regenerate with `python points/dump_atim_c.py --only mtv_4096_1_4096.reproduced`.
...
int main() {
  ...
  for (int32_t k_1_0 = 0; k_1_0 < 16; ++k_1_0) {
    ...
    for (int32_t ax0 = 0; ax0 < 2; ++ax0) {
      ...
      mram_read((__mram_ptr void*)(A + (((tasklet_id * 512) + (k_1_0 * 32)) + cse_var_1)), A_local + cse_var_1, 64);
    }
    ...
    for (int32_t k_1_1 = 0; k_1_1 < 4; ++k_1_1) {
      ...
      for (int32_t i_4 = 0; i_4 < 2; ++i_4) {
        #pragma clang loop unroll(full)
        for (int32_t k_1_2 = 0; k_1_2 < 4; ++k_1_2) {
          int32_t cse_var_2 = (k_1_1 * 4);
          C_rf_global_local[i_4] = (C_rf_global_local[i_4] + (A_local[(((i_4 * 16) + cse_var_2) + k_1_2)] * B_local[(cse_var_2 + k_1_2)]));
        }
      }
    }
  }
