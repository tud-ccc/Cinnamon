# TIR those decisions produced, for ('gemv', 4096, 1, 4096) (reproduced).
                        ...
                        for i_2 in range(1):
                            for i_3_init, i_4_init in T.grid(2, 1):
                                with T.block("C_rf_init"):
                                    v_i = T.axis.spatial(4096, i_0 * 32 + i_1 * 2 + i_2 * 2 + i_3_init + i_4_init)
                            ...
                            for k_1_0, i_3, k_1_1 in T.grid(16, 2, 1):
                                for ax0, ax1 in T.grid(1, 16):
                                    with T.block("A_local"):
                                        v0 = T.axis.spatial(4096, i_0 * 32 + i_1 * 2 + i_3 + ax0)
                                        v1 = T.axis.spatial(4096, k_0 * 256 + k_1_0 * 16 + ax1)
                                        T.reads(A[v0, v1])
                                ...
                                for ax0 in range(16):
                                    with T.block("B_local"):
                                        v0 = T.axis.spatial(4096, k_0 * 256 + k_1_0 * 16 + ax0)
                                        T.reads(B[v0])
                                        T.writes(B_local[v0])
                                        T.block_attr({"meta_schedule.cooperative_fetch": 1})
                                        B_local[v0] = B[v0]
                                for i_4, k_1_2 in T.grid(1, 16):
                                    with T.block("C_rf_update"):
                                        v_i = T.axis.spatial(4096, i_0 * 32 + i_1 * 2 + i_2 * 2 + i_3 + i_4)
                                        vk_0 = T.axis.spatial(16, k_0)
                                        vk_1 = T.axis.reduce(256, k_1_0 * 16 + k_1_1 * 16 + k_1_2)
                                        T.reads(C_rf_global_local[vk_0, v_i], A_local[v_i, vk_0 * 256 + vk_1], B_local[vk_0 * 256 + vk_1])
                                        T.writes(C_rf_global_local[vk_0, v_i])
                                        T.block_attr({"meta_schedule.meta_schedule_rfactor_producer_block": T.int64(1), "meta_schedule.tiling_structure": "SSSRSRSR"})
                                        C_rf_global_local[vk_0, v_i] = C_rf_global_local[vk_0, v_i] + alpha_val * A_local[v_i, vk_0 * 256 + vk_1] * B_local[vk_0 * 256 + vk_1]
                            for ax0, ax1 in T.grid(1, 2):
                                with T.block("C_rf_global_local"):
                                    v0 = T.axis.spatial(16, k_0 + ax0)
                                    v1 = T.axis.spatial(4096, i_0 * 32 + i_1 * 2 + ax1)
