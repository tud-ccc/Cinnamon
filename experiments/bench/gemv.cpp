...
extern "C" void BENCH_FN(float *, float *, float *);
...
int main(int argc, char *argv[]) {
    ...
    GemvSpec s = spec();
    srand(0);
    float *A   = alloc_f32(s.m * s.n);
    float *x   = alloc_f32(s.n);
    float *out = alloc_f32(s.m);   // output buffer; reused across iters
    ...
    snprintf(total_path, sizeof(total_path), "%s/" TOSTR(BENCH_FN) "_total.csv", out_dir);
