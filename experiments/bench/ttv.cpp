// Bench driver for ttv (prim_ttv) functions.
//
...
static TtvSpec spec() {
  ...
  if (!strcmp(fn, "ttv_4MB"))
    return {32, 64, 512};
  if (!strcmp(fn, "ttv_64MB"))
    return {128, 256, 512};
  if (!strcmp(fn, "ttv_256MB"))
    return {256, 512, 512};
  if (!strcmp(fn, "ttv_512MB"))
    return {512, 512, 512};
  fprintf(stderr, "ttv.cpp: unknown function '%s' — add it to spec()\n", fn);
  exit(1);
}
