...
class Config:
    ...
    params: dict                  # dpus/tasklets (+ whatever else); informational, e.g. DPU-cap accounting
    fn_module: pathlib.Path       # single-function source .mlir
    prim: str                     # e.g. "gemv" / "red", passed as bench-single's PRIM=
    lower: Callable[[pathlib.Path, pathlib.Path, pathlib.Path], subprocess.CompletedProcess]
...
def compile_config(config: Config, *, compile_root: pathlib.Path) -> CompiledConfig:
    config_dir = pathlib.Path(compile_root) / config.system / config.fn_name / config.label
    config_dir.mkdir(parents=True, exist_ok=True)

    with open(config_dir / "config.csv", "w", newline="") as f:
        writer = csv.DictWriter(f, fieldnames=["system", "fn_name", "label", *config.params.keys()])
        writer.writeheader()
        writer.writerow({"system": config.system, "fn_name": config.fn_name,
...
def run_config(compiled: CompiledConfig, *, run_root: pathlib.Path, iters: int) -> RunResult:
    if not compiled.ok:
    ...
    output_dir = pathlib.Path(run_root) / cfg.system / cfg.fn_name / cfg.label / "output"
    output_dir.mkdir(parents=True, exist_ok=True)

    r = subprocess.run(
        [str(bench_bin), str(output_dir), str(iters)],
        capture_output=True, text=True, cwd=str(bin_dir / cfg.fn_name),
    )
    if r.returncode != 0:
...
def discover_compiled(configs: list[Config], *, compile_root: pathlib.Path) -> list[CompiledConfig]:
    ...
    for config in configs:
        ...
        if bench_bin.exists():
            ...
            results.append(CompiledConfig(config, config_dir, False,
...
def compile_configs(configs: list[Config], *, compile_root: pathlib.Path,
                     workers: int = 8) -> list[CompiledConfig]:
    ...
    compiled = parallel.run_parallel(
        ...
        workers=workers, desc="compile", use_threads=True,
    )
    for c in compiled:
        if not c.ok:
            print(f"  FAIL compile: {c.config.system} {c.config.fn_name} {c.config.label}: {c.error}")
    return compiled


def run_configs(compiled: list[CompiledConfig], *, run_root: pathlib.Path,
                 iters: int = 10) -> list[RunResult]:
    ...
    for c in tqdm(compiled, desc="run"):
        ...
        if not r.ok and is_dpu_allocation_error(r.error):
            r = run_config(c, run_root=run_root, iters=iters)
        if not r.ok:
            print(f"  FAIL run: {c.config.system} {c.config.fn_name} {c.config.label}: {r.error[:200]}")
        results.append(r)
    return results
