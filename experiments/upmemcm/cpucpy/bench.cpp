...
#endif
#ifndef T_VAL
#error "T_VAL must be defined at compile time (-DT_VAL=<chunk_size>)"
#endif

extern "C" int32_t bench(int32_t *, int32_t *);

static constexpr size_t M = (size_t)M_VAL;
static constexpr size_t T = (size_t)T_VAL;
// Number of memcpy iterations inside one bench() call: loop runs M/(2T) times.
static constexpr size_t N_LOOP = M / (2 * T);

static constexpr int WARMUP = 5;
static constexpr int BENCH_REPS = 20;
...
int main(int argc, char *argv[]) {
  ...
  for (int rep = 0; rep < BENCH_REPS; rep++) {
    ...
    double time_iter_ns = (double)elapsed_ns / (double)N_LOOP;
    fprintf(f, "%zu,%zu,%.3f\n", M, T, time_iter_ns);
  }
