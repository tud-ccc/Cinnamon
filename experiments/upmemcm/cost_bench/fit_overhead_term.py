...
def apply_correction(dpus: np.ndarray, mramCols: np.ndarray,
                      intercept: float, coef: list[float], feature_fn) -> np.ndarray:
...
def fit_template(dpus: np.ndarray, mramCols: np.ndarray, residual: np.ndarray,
                  weight: np.ndarray, feature_fn) -> dict:
    ...
    return {"intercept": float(coef_full[0]), "coef": list(coef_full[1:]),
            "pred": X_full @ coef_full}


def fit_all_templates(dpus: np.ndarray, mramCols: np.ndarray, residual: np.ndarray,
                       weight: np.ndarray) -> pd.DataFrame:
    rows = []
    for name, feature_fn in TEMPLATES.items():
        ...
        rows.append({"template": name, "weighted_rmse_ms": rmse,
                     "intercept": fit["intercept"], "coef": fit["coef"]})
...
def fit_smac(
    ...
    def objective(config, seed: int = 0) -> float:
        ...
        rhos, norm_rmses = [], []
        for (dpus, mramCols, predicted_ms, measured_ms, _), rmse_w, baseline_rmse in \
                zip(datasets, rmse_weights, baseline_rmses):
    ...
    scenario = Scenario(cs, n_trials=n_trials, seed=seed, deterministic=True,
...
def make_fit_plot(dpus: np.ndarray, mramCols: np.ndarray, residual: np.ndarray,
                   weight: np.ndarray, best_name: str, out_path: pathlib.Path, title: str):
    fit = fit_template(dpus, mramCols, residual, weight, TEMPLATES[best_name])
    order = np.argsort(dpus)
    fig, ax = plt.subplots(figsize=(6, 4.5))
    ax.scatter(dpus, residual, s=10, alpha=0.4, color="darkorange",
...
def make_calibration_scatter_plot(dpus: np.ndarray,
...
def main():
    ...
    parser.add_argument("--top-quantile", type=float, default=0.20,
                        help="Fraction of top configs used for Spearman ρ ranking metric "
                             "(default: 0.20 = top 20%%)")
    parser.add_argument("--smac-trials", type=int, default=200,
    ...
    parser.add_argument("--no-rmse-cost-weighted", dest="rmse_cost_weighted",
    ...
    for fn_name, pool_csv in find_function_pools(pathlib.Path(args.kernel_oracle)):
        ...
        make_calibration_scatter_plot(
            dpus, predicted_ms, measured_ms, fit["pred"],
            f"NNLS: {best}", top_q,
    ...
    fit_pooled  = fit_template(dpus_pooled, mramCols_pooled, residual_pooled,
                               weight_pooled, TEMPLATES[best_pooled])
    rho_pooled  = spearman_topk_rho(predicted_pooled + fit_pooled["pred"], measured_pooled, top_q)
    print(f"  NNLS best pooled ({best_pooled}): Spearman ρ@top-{top_q:.0%} = {rho_pooled:.3f}")
    ...
    make_calibration_scatter_plot(
        dpus_pooled, predicted_pooled, measured_pooled, fit_pooled["pred"],
    ...
    print(f"  CV Spearman ρ@top-{top_q:.0%}: {smac_result['cv_spearman_rho']:.3f}  "
          f"(vs NNLS pooled: {rho_pooled:.3f})")
    print(f"  CV normalised RMSE : {smac_result['cv_normalized_rmse']:.3f}  "
          f"(1.0 = no improvement over no correction)")

    smac_correction = apply_correction(
        dpus_pooled, mramCols_pooled,
        smac_result["intercept"], smac_result["coef"],
        TEMPLATES[smac_result["template"]],
    )
    make_calibration_scatter_plot(
        dpus_pooled, predicted_pooled, measured_pooled, smac_correction,
