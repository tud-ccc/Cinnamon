...
def load_pool(csv_path: Path, stats: dict):
    ...
    valid_mask = (
        df["valid"].eq(1) if "valid" in df.columns else pd.Series(True, index=df.index)
    )
    ...
    print(
        f"Loaded {n_total} configs total: {n_valid} valid, {n_obs} with observed cost"
    )
...
def axial_pairs(good, dim_cols, sub_maps, dim, lag=1):
    ...
    right = g.copy()
    right["_sub"] -= (
        lag  # shift so: right._sub_shifted == left._sub iff orig = left+lag
    )
...
def isoproduct_pairs(good, dim_cols, dim_i, dim_j):
    ...
    merged = g.merge(
        g[join_cols + ["_id", "logcost"]], on=join_cols, suffixes=("_a", "_b")
    )
...
def plot_cost_dist(good, out_dir):
    ...
    ax.hist(
        logc, bins=50, color="steelblue", alpha=0.7, edgecolor="white", linewidth=0.4
    )
    colors = plt.cm.RdYlGn_r(np.linspace(0.1, 0.9, len(pcts)))
    for p, v, c in zip(pcts, vals, colors):
        ax.axvline(
            v, color=c, linewidth=1.4, linestyle="--", label=f"p{p} = {10**v:.2g}"
        )
...
def plot_marginals(good, dim_cols, out_dir):
    ...
    fig.suptitle(
        "Per-dimension marginals  (mean ± IQR, other dims averaged out)", fontsize=12
    )
    fig.tight_layout()
...
def plot_roughness(good, dim_cols, sub_maps, out_dir):
    ...
    if not data:
        return {
            "_name": None,
            "_log": ["  (no axis-aligned pairs found — skipping roughness)"],
        }
    ...
    table_rows = [
        "| Dimension | Mean \\|Δlog₁₀f\\| | Std | Pairs |",
        "|-----------|-----------------|-----|-------|",
    ]
    for dim in dims_sorted:
        n_pairs = len(data[dim])
        log.append(
            f"      {dim:20s}  {means[dim]:.4f} ± {data[dim].std():.4f}  (n={n_pairs})"
        )
        table_rows.append(
    ...
    for ci, dim in enumerate(dim_cols):
        ...
        if lags:
            ax.plot(
                ...
                markersize=4,
                linewidth=1.5,
                color=cmap(ci / max(len(dim_cols), 1)),
                label=dim,
            )
...
def plot_2d_marginals(good, dim_cols, out_dir):
    ...
    for ax, (di, dj) in zip(axes.flatten(), pairs):
        piv = good.pivot_table(
            index=di, columns=dj, values="logcost", aggfunc="mean"
    ...
    fig.suptitle(
        "2-D marginals  (mean log₁₀ cost, other dims averaged out)\n"
        "Shared colour scale — panels are directly comparable.",
        fontsize=11,
    )
    fig.tight_layout()
...
def plot_compensation(good, dim_cols, sub_maps, out_dir):
    ...
    if not rows:
        return {
            "_name": None,
            "_log": ["  (no pairs had enough data — skipping compensation)"],
        }

    results = pd.DataFrame(rows).sort_values("ratio")

    fig, (ax_bar, ax_scatter) = plt.subplots(
        1, 2, figsize=(14, max(4, len(rows) * 0.55 + 2))
    )
    ...
    ax_scatter.plot(
        [0, lim], [0, lim], "k--", linewidth=0.8, alpha=0.5, label="ratio = 1"
    )
    ...
    log = [
        f"    {'Pair':25s}  {'Ratio':>7s}  {'R²(prod)':>9s}  "
        f"{'n_axis':>8s}  {'n_prod':>8s}"
    ]
    table_rows = [
        "| Pair | E_prod/E_axis | R² (product) | n_axis | n_prod |",
        "|------|-------------|--------------|--------|--------|",
    ]
    for _, row in results.iterrows():
        flag = "  ← strong" if row["ratio"] < 0.5 else ""
        log.append(
            f"    {row['pair']:25s}  {row['ratio']:7.3f}  {row['explained']:9.3f}"
...
def plot_topk(good, dim_cols, out_dir, top_frac):
    ...
    for ax, dim in zip(axes.flatten(), dim_cols):
        ...
        ax.bar(
            x + w / 2, top_f, w, label=f"top {top_frac:.0%}", color="tomato", alpha=0.75
        )
        ax.set_xticks(x)
    ...
    fig.suptitle(
        f"Top-{top_frac:.0%} concentration  "
        f"(n_top = {top_n} / n_all = {len(good)})\n"
        "Large shift → good configs concentrate at specific dim values.",
        fontsize=11,
    )
    fig.tight_layout()
...
def _analyze_csv(csv_path: Path, top_frac: float, max_lag: int, out_dir: Path) -> str:
    ...
    stats.setdefault(
        "compensation_table", "_Not enough data for compensation analysis._"
    )
...
def main():
    ...
    if len(csv_paths) == 1:
        ...
        plot_tasks = {
            ...
            "variogram": (
                plot_variogram,
                (good, dim_cols, sub_maps, out_dir, args.max_lag),
            ),
            "2d_marginals": (plot_2d_marginals, (good, dim_cols, out_dir)),
            "compensation": (plot_compensation, (good, dim_cols, sub_maps, out_dir)),
        ...
        with ProcessPoolExecutor(max_workers=workers) as executor:
            for p in csv_paths:
                ...
                futures[
                    executor.submit(_analyze_csv, p, args.top_frac, args.max_lag, p_out)
                ] = p
            for future in tqdm(
                as_completed(futures), total=len(futures), desc="problems"
            ):
                p = futures[future]
                ex = future.exception()
                if ex:
