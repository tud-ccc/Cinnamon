...
static inline int8_t aimc_quantize_scalar(float x, float scale,
                                          int8_t zero_point) {
...
static inline float aimc_dequantize_scalar(int8_t q, float scale,
                                           int8_t zero_point) {
  return ((int32_t)q - (int32_t)zero_point) * scale;
}

// Vector quantize: float[N] -> int8[N]
static inline void aimc_quantize_vector(int size, const float *x, int8_t *q,
...
static inline void aimc_dequantize_vector(int size, const int8_t *q, float *x,
                                          float scale, int8_t zero_point) {
  for (int i = 0; i < size; i++) {
