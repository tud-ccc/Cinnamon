...
inline void queueVector(int size, int8_t *v, int tid = 0) {
  ...
  // Queue the final pack. If size==0, skip; if size%4==0, this queues the last
  // full pack.
  if (size > 0) {
    aimcQueue(tmp, tid);
  }
...
template <typename T>
inline void queueVector(int size, T max, T *v, int tid = 0) {
#if defined(LOOSELY_COUPLED_MMIO)
  for (int i = 0; i < size; i++) {
