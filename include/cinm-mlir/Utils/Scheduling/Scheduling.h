...
template <typename ComputeResource> struct ScheduledOperation {
  Operation *operation;
...
template <typename ComputeResource> struct SchedulingHooks {
...
void applyScheduling(PatternRewriter &rewriter, ValueRange rootValues,
  ...
  while (!schedule.empty()) {
    ...
    for (auto it = schedule.begin(); it != schedule.end();) {
      ...
      auto *barrierOp =
          hooks.barrierInserter(rewriter, *scheduledOp->operation);
