// OpenLLaMA 3B, integer-only single-token decode, for the whole-program
// experiment: the decode-shaped end-to-end model between llama2_110M_w8a8
// and llama2_7B_w8a8. Same program as llama2_110M_w8a8.mlir with the
// dimensions of OpenLLaMA 3B v2 (Geng and Liu 2023), the Llama-architecture
// 3B whose shape this program has -- one key/value head per query head and
// a 32000-entry vocabulary (Llama 3.2 3B has neither); the files differ in
// nothing but shapes.
//
// Model:
//   dim        (H)  3200
//   hidden     (F)  8640
//   layers     (L)  26
//   heads      (A)  32    -> head size 100 (kv heads = heads, no grouping)
//   vocab      (V)  32000
//   sequence   (N)  1024  (the KV cache length, as for the other sizes)
...
// parameters are. The Q, K, V projections are one 9600x3200 weight, stored
...
//
// Weights: 102 MB of embeddings, 26 * (9600 + 3200 + 3 * 8640) * 3200 =
// 3.22 GB of layer weights, 105 MB of classifier, 13.1 MB of RoPE tables --
// 3.44 GB of i8 (and Q14 i32 tables) that stays resident on the device, 1.7
// MB per DPU of a 2048-DPU array; the caches (170 MB, i8) are per-call
// state. Per layer the QKV weight is 31 MB and w1/w2/w3 28 MB each, above
// the point where streaming a weight from host DRAM costs more than a
...
func.func @llama_3B_w8a8(
	...
	%kc : tensor<26x1024x3200xi8> {bufferization.writable = true, bufferization.buffer_layout = affine_map<(i,j,k) -> (i,j,k)>},
	%vc : tensor<26x1024x3200xi8> {bufferization.writable = true, bufferization.buffer_layout = affine_map<(i,j,k) -> (i,j,k)>},
	// RoPE, Q14: row p holds cos/sin(p * freq(j)) for the 1600 rotated pairs
	%rope_cos : tensor<1024x1600xi32> {cinm.static, bufferization.buffer_layout = affine_map<(i,j) -> (i,j)>},
	%rope_sin : tensor<1024x1600xi32> {cinm.static, bufferization.buffer_layout = affine_map<(i,j) -> (i,j)>},
	// weights, stored [layer][out][in]
	%embedding_table : tensor<32000x3200xi8> {cinm.static, bufferization.buffer_layout = affine_map<(i,j) -> (i,j)>},
	%rms_att_weights : tensor<26x3200xi32> {cinm.static, bufferization.buffer_layout = affine_map<(i,j) -> (i,j)>},
	%wqkv : tensor<26x9600x3200xi8> {cinm.static, bufferization.buffer_layout = affine_map<(i,j,k) -> (i,j,k)>},
	%wo : tensor<26x3200x3200xi8> {cinm.static, bufferization.buffer_layout = affine_map<(i,j,k) -> (i,j,k)>},
	%rms_ffn_weights : tensor<26x3200xi32> {cinm.static, bufferization.buffer_layout = affine_map<(i,j) -> (i,j)>},
	%w1 : tensor<26x8640x3200xi8> {cinm.static, bufferization.buffer_layout = affine_map<(i,j,k) -> (i,j,k)>},
	%w2 : tensor<26x3200x8640xi8> {cinm.static, bufferization.buffer_layout = affine_map<(i,j,k) -> (i,j,k)>},
	%w3 : tensor<26x8640x3200xi8> {cinm.static, bufferization.buffer_layout = affine_map<(i,j,k) -> (i,j,k)>},
	%rms_final_weight : tensor<3200xi32> {cinm.static, bufferization.buffer_layout = affine_map<(i) -> (i)>},
	%wcls : tensor<32768x3200xi8> {cinm.static, bufferization.buffer_layout = affine_map<(i,j) -> (i,j)>}
	...
	%x_row = tensor.extract_slice %embedding_table [%token_idx, 0] [1, 3200] [1, 1] : tensor<32000x3200xi8> to tensor<3200xi8>
	%x_in = func.call @widen_v(%x_row) : (tensor<3200xi8>) -> tensor<3200xi32>

	%cos = tensor.extract_slice %rope_cos [%pos_idx, 0] [1, 1600] [1, 1] : tensor<1024x1600xi32> to tensor<1600xi32>
	%sin = tensor.extract_slice %rope_sin [%pos_idx, 0] [1, 1600] [1, 1] : tensor<1024x1600xi32> to tensor<1600xi32>

	// ---- decoder layers ----------------------------------------------------
	%xL, %kcL, %vcL = scf.for %layer = %c0 to %c26 step %c1
			iter_args(%x = %x_in, %kc0 = %kc, %vc0 = %vc)
			-> (tensor<3200xi32>, tensor<26x1024x3200xi8>, tensor<26x1024x3200xi8>) {
		%rms_att_w = tensor.extract_slice %rms_att_weights [%layer, 0] [1, 3200] [1, 1] : tensor<26x3200xi32> to tensor<3200xi32>
		%xb = func.call @irmsnorm(%x, %rms_att_w) : (tensor<3200xi32>, tensor<3200xi32>) -> tensor<3200xi8>

		// Q, K, V in one GEMV against the fused weight, requantized together
		%wqkv_l = tensor.extract_slice %wqkv [%layer, 0, 0] [1, 9600, 3200] [1, 1, 1] : tensor<26x9600x3200xi8> to tensor<9600x3200xi8>
		%qkv_acc = cinm.op.gemv %wqkv_l, %xb : tensor<9600x3200xi8>, tensor<3200xi8> -> tensor<9600xi32>
		%qkv = func.call @requant_qkv(%qkv_acc) : (tensor<9600xi32>) -> tensor<9600xi8>
		%q = tensor.extract_slice %qkv [0] [3200] [1] : tensor<9600xi8> to tensor<3200xi8>
		%k = tensor.extract_slice %qkv [3200] [3200] [1] : tensor<9600xi8> to tensor<3200xi8>
		%v = tensor.extract_slice %qkv [1536] [3200] [1] : tensor<9600xi8> to tensor<3200xi8>

		// RoPE on q and k, then the caches take this position's k and v
		%q2 = func.call @irope(%q, %cos, %sin) : (tensor<3200xi8>, tensor<1600xi32>, tensor<1600xi32>) -> tensor<3200xi8>
		%k2 = func.call @irope(%k, %cos, %sin) : (tensor<3200xi8>, tensor<1600xi32>, tensor<1600xi32>) -> tensor<3200xi8>
		%kc1 = tensor.insert_slice %k2 into %kc0 [%layer, %pos_idx, 0] [1, 1, 3200] [1, 1, 1] : tensor<3200xi8> into tensor<26x1024x3200xi8>
		%vc1 = tensor.insert_slice %v into %vc0 [%layer, %pos_idx, 0] [1, 1, 3200] [1, 1, 1] : tensor<3200xi8> into tensor<26x1024x3200xi8>

		// attention over the whole cache of this layer
		%lkc = tensor.extract_slice %kc1 [%layer, 0, 0] [1, 1024, 3200] [1, 1, 1] : tensor<26x1024x3200xi8> to tensor<1024x3200xi8>
		%lvc = tensor.extract_slice %vc1 [%layer, 0, 0] [1, 1024, 3200] [1, 1, 1] : tensor<26x1024x3200xi8> to tensor<1024x3200xi8>
		%ctx = func.call @imha(%q2, %lkc, %lvc, %attn_mask)
			: (tensor<3200xi8>, tensor<1024x3200xi8>, tensor<1024x3200xi8>, tensor<1024xi32>) -> tensor<3200xi8>
		...
		%wo_l = tensor.extract_slice %wo [%layer, 0, 0] [1, 3200, 3200] [1, 1, 1] : tensor<26x3200x3200xi8> to tensor<3200x3200xi8>
		%ao_acc = cinm.op.gemv %wo_l, %ctx : tensor<3200x3200xi8>, tensor<3200xi8> -> tensor<3200xi32>
		%ao = func.call @requant_v(%ao_acc) : (tensor<3200xi32>) -> tensor<3200xi8>
		%ao_w = func.call @widen_v(%ao) : (tensor<3200xi8>) -> tensor<3200xi32>
		%x1 = cinm.op.elementwise add %x, %ao_w : tensor<3200xi32>

		// feed-forward: w2(silu(w1 x) * w3 x), 3200 -> 8640 -> 3200
		%rms_ffn_w = tensor.extract_slice %rms_ffn_weights [%layer, 0] [1, 3200] [1, 1] : tensor<26x3200xi32> to tensor<3200xi32>
		%xf = func.call @irmsnorm(%x1, %rms_ffn_w) : (tensor<3200xi32>, tensor<3200xi32>) -> tensor<3200xi8>
		%w1_l = tensor.extract_slice %w1 [%layer, 0, 0] [1, 8640, 3200] [1, 1, 1] : tensor<26x8640x3200xi8> to tensor<8640x3200xi8>
		%w3_l = tensor.extract_slice %w3 [%layer, 0, 0] [1, 8640, 3200] [1, 1, 1] : tensor<26x8640x3200xi8> to tensor<8640x3200xi8>
		%w2_l = tensor.extract_slice %w2 [%layer, 0, 0] [1, 3200, 8640] [1, 1, 1] : tensor<26x3200x8640xi8> to tensor<3200x8640xi8>
		%h1_acc = cinm.op.gemv %w1_l, %xf : tensor<8640x3200xi8>, tensor<3200xi8> -> tensor<8640xi32>
		%h3_acc = cinm.op.gemv %w3_l, %xf : tensor<8640x3200xi8>, tensor<3200xi8> -> tensor<8640xi32>
		%h1 = func.call @requant_f(%h1_acc) : (tensor<8640xi32>) -> tensor<8640xi8>
		%h3 = func.call @requant_f(%h3_acc) : (tensor<8640xi32>) -> tensor<8640xi8>
		%hb = func.call @iswiglu(%h1, %h3) : (tensor<8640xi8>, tensor<8640xi8>) -> tensor<8640xi8>
		%f_acc = cinm.op.gemv %w2_l, %hb : tensor<3200x8640xi8>, tensor<8640xi8> -> tensor<3200xi32>
		%f = func.call @requant_v(%f_acc) : (tensor<3200xi32>) -> tensor<3200xi8>
	...
	%xn = func.call @irmsnorm(%xL, %rms_final_weight) : (tensor<3200xi32>, tensor<3200xi32>) -> tensor<3200xi8>
	%logits_p = cinm.op.gemv %wcls, %xn : tensor<32768x3200xi8>, tensor<3200xi8> -> tensor<32768xi32>
...
//
// Per head: scores = K_h q_h / sqrt(100) over all 1024 cache rows (a GEMV
...
// 1 x 1024 by 1024 x 100 product. Heads accumulate into one i32 context and
// are requantized together after the loop.
// ===========================================================================
func.func @imha(%q : tensor<3200xi8>, %kc : tensor<1024x3200xi8>, %vc : tensor<1024x3200xi8>,
                %mask : tensor<1024xi32>) -> tensor<3200xi8> {
	%c0 = arith.constant 0 : index
	%c100 = arith.constant 100 : index
	%c3200 = arith.constant 3200 : index
	%init_ctx = tensor.empty() : tensor<3200xi32>

	%ctx_acc = scf.for %h = %c0 to %c3200 step %c100 iter_args(%ctx_i = %init_ctx) -> (tensor<3200xi32>) {
		%qs = tensor.extract_slice %q [%h] [100] [1] : tensor<3200xi8> to tensor<100xi8>
		%kh = tensor.extract_slice %kc [0, %h] [1024, 100] [1, 1] : tensor<1024x3200xi8> to tensor<1024x100xi8>
		%vh = tensor.extract_slice %vc [0, %h] [1024, 100] [1, 1] : tensor<1024x3200xi8> to tensor<1024x100xi8>

		%sc_acc = cinm.op.gemv %kh, %qs : tensor<1024x100xi8>, tensor<100xi8> -> tensor<1024xi32>
		...
		%ch2d = cinm.op.gemm %p2d, %vh : tensor<1x1024xi8>, tensor<1024x100xi8> -> tensor<1x100xi32>
		%ch = tensor.collapse_shape %ch2d [[0, 1]] : tensor<1x100xi32> into tensor<100xi32>
		%ctx_o = tensor.insert_slice %ch into %ctx_i [%h] [100] [1] : tensor<100xi32> into tensor<3200xi32>
		scf.yield %ctx_o : tensor<3200xi32>
	} {unroll_heads}

	%ctx = func.call @requant_v(%ctx_acc) : (tensor<3200xi32>) -> tensor<3200xi8>
	return %ctx : tensor<3200xi8>
}
...
// ===========================================================================
func.func @irope(%q : tensor<3200xi8>, %cos : tensor<1600xi32>, %sin : tensor<1600xi32>) -> tensor<3200xi8> {
	%q0 = tensor.extract_slice %q [0] [1600] [2] : tensor<3200xi8> to tensor<1600xi8>
	%q1 = tensor.extract_slice %q [1] [1600] [2] : tensor<3200xi8> to tensor<1600xi8>
	%init = tensor.empty() : tensor<1600xi8>
	%r0 = linalg.map ins(%q0, %q1, %cos, %sin : tensor<1600xi8>, tensor<1600xi8>, tensor<1600xi32>, tensor<1600xi32>) outs(%init : tensor<1600xi8>)
	...
	%r1 = linalg.map ins(%q0, %q1, %cos, %sin : tensor<1600xi8>, tensor<1600xi8>, tensor<1600xi32>, tensor<1600xi32>) outs(%init : tensor<1600xi8>)
		(%a : i8, %b : i8, %c : i32, %s : i32, %o : i8) {
			%q14 = arith.constant 14 : i32
			%a32 = arith.extsi %a : i8 to i32
			%b32 = arith.extsi %b : i8 to i32
	...
	%o0 = tensor.insert_slice %r0 into %q [0] [1600] [2] : tensor<1600xi8> into tensor<3200xi8>
	%o1 = tensor.insert_slice %r1 into %o0 [1] [1600] [2] : tensor<1600xi8> into tensor<3200xi8>
	return %o1 : tensor<3200xi8>
}

// ===========================================================================
// I-RMSNorm  (the sqrt of I-BERT 3.4, without the mean)
//
//   ss    = sum((x >> 3)^2)            -- pre-shifted so 3200 squares fit i32
...
// ===========================================================================
func.func @irmsnorm(%x : tensor<3200xi32>, %w : tensor<3200xi32>) -> tensor<3200xi8> {
	%init = tensor.empty() : tensor<3200xi32>
	%init_o = tensor.empty() : tensor<3200xi8>
	%pre = arith.constant 3 : i32
	%h = arith.constant 3200 : i32
	...
	%xs = linalg.map ins(%x : tensor<3200xi32>) outs(%init : tensor<3200xi32>)
		(%v : i32, %o : i32) {
			%r = arith.shrsi %v, %pre : i32
			linalg.yield %r : i32
		}
	%sq = cinm.op.elementwise mul %xs, %xs : tensor<3200xi32>
	%ss = cinm.op.reduce add (%sq) : tensor<3200xi32> -> i32
	...
	%out = linalg.map ins(%x, %w : tensor<3200xi32>, tensor<3200xi32>) outs(%init_o : tensor<3200xi8>)
		(%v : i32, %g : i32, %o : i8) {
...
// ===========================================================================
func.func @iswiglu(%h1 : tensor<8640xi8>, %h3 : tensor<8640xi8>) -> tensor<8640xi8> {
	%init_o = tensor.empty() : tensor<8640xi8>
	%r = linalg.map ins(%h1, %h3 : tensor<8640xi8>, tensor<8640xi8>) outs(%init_o : tensor<8640xi8>)
		(%a : i8, %g : i8, %o : i8) {
			%ln2 = arith.constant 11 : i32
			%pa = arith.constant 5874 : i32
			%pb = arith.constant 22168 : i32
...
// header describes. The product genuinely needs 64 bits: an 8640-deep int8
// accumulation reaches ~1.4e8, and M is ~2e4.
...
func.func @requant_v(%x : tensor<3200xi32>) -> tensor<3200xi8> {
	%init = tensor.empty() : tensor<3200xi8>
	%r = linalg.map ins(%x : tensor<3200xi32>) outs(%init : tensor<3200xi8>)
		(%v : i32, %o : i8) {
			%y = func.call @requant_scalar(%v) : (i32) -> i8
			linalg.yield %y : i8
		}
	return %r : tensor<3200xi8>
}

func.func @requant_qkv(%x : tensor<9600xi32>) -> tensor<9600xi8> {
	%init = tensor.empty() : tensor<9600xi8>
	%r = linalg.map ins(%x : tensor<9600xi32>) outs(%init : tensor<9600xi8>)
		(%v : i32, %o : i8) {
			%y = func.call @requant_scalar(%v) : (i32) -> i8
			linalg.yield %y : i8
		}
	return %r : tensor<9600xi8>
}

func.func @requant_f(%x : tensor<8640xi32>) -> tensor<8640xi8> {
	%init = tensor.empty() : tensor<8640xi8>
	%r = linalg.map ins(%x : tensor<8640xi32>) outs(%init : tensor<8640xi8>)
		(%v : i32, %o : i8) {
			%y = func.call @requant_scalar(%v) : (i32) -> i8
			linalg.yield %y : i8
		}
...
func.func @widen_v(%x : tensor<3200xi8>) -> tensor<3200xi32> {
	%init = tensor.empty() : tensor<3200xi32>
	%r = linalg.map ins(%x : tensor<3200xi8>) outs(%init : tensor<3200xi32>)
		(%v : i8, %o : i32) {
			%y = arith.extsi %v : i8 to i32
			linalg.yield %y : i32
		}
	return %r : tensor<3200xi32>
}

// The attention scores keep the accumulator width but change scale: this
// folds in 1/sqrt(100) = 1/10, taken as 1/8 -- a placeholder like the
