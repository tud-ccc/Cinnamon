// Llama-2 110M, integer-only single-token decode, for the whole-program
// experiment: the decode-shaped end-to-end model beside roberta_base.
//
// Model: the tinyllamas/stories110M configuration (llama2.c), the same one
// llama2_110M.mlir spells in f32:
//   dim        (H)  768
//   hidden     (F)  2048
//   layers     (L)  6
//   heads      (A)  6     -> head size 48 (kv heads = heads, no grouping)
//   vocab      (V)  32000
//   sequence   (N)  1024  (the KV cache length)
...
// parameters are. The Q, K, V projections are one 2304x768 weight, stored
...
//
// Weights: 24.6 MB of embeddings, 6 * (2304 + 768 + 3 * 2048) * 768 =
// 42.5 MB of layer weights, 25.2 MB of classifier, 3.1 MB of RoPE tables --
...
func.func @llama2_110M_w8a8(
	...
	%kc : tensor<6x1024x768xi8> {bufferization.writable = true, bufferization.buffer_layout = affine_map<(i,j,k) -> (i,j,k)>},
	%vc : tensor<6x1024x768xi8> {bufferization.writable = true, bufferization.buffer_layout = affine_map<(i,j,k) -> (i,j,k)>},
	// RoPE, Q14: row p holds cos/sin(p * freq(j)) for the 384 rotated pairs
	%rope_cos : tensor<1024x384xi32> {cinm.static, bufferization.buffer_layout = affine_map<(i,j) -> (i,j)>},
	%rope_sin : tensor<1024x384xi32> {cinm.static, bufferization.buffer_layout = affine_map<(i,j) -> (i,j)>},
	// weights, stored [layer][out][in]
	%embedding_table : tensor<32000x768xi8> {cinm.static, bufferization.buffer_layout = affine_map<(i,j) -> (i,j)>},
	%rms_att_weights : tensor<6x768xi32> {cinm.static, bufferization.buffer_layout = affine_map<(i,j) -> (i,j)>},
	%wqkv : tensor<6x2304x768xi8> {cinm.static, bufferization.buffer_layout = affine_map<(i,j,k) -> (i,j,k)>},
	%wo : tensor<6x768x768xi8> {cinm.static, bufferization.buffer_layout = affine_map<(i,j,k) -> (i,j,k)>},
	%rms_ffn_weights : tensor<6x768xi32> {cinm.static, bufferization.buffer_layout = affine_map<(i,j) -> (i,j)>},
	%w1 : tensor<6x2048x768xi8> {cinm.static, bufferization.buffer_layout = affine_map<(i,j,k) -> (i,j,k)>},
	%w2 : tensor<6x768x2048xi8> {cinm.static, bufferization.buffer_layout = affine_map<(i,j,k) -> (i,j,k)>},
	%w3 : tensor<6x2048x768xi8> {cinm.static, bufferization.buffer_layout = affine_map<(i,j,k) -> (i,j,k)>},
	%rms_final_weight : tensor<768xi32> {cinm.static, bufferization.buffer_layout = affine_map<(i) -> (i)>},
	%wcls : tensor<32768x768xi8> {cinm.static, bufferization.buffer_layout = affine_map<(i,j) -> (i,j)>}
	...
	%x_row = tensor.extract_slice %embedding_table [%token_idx, 0] [1, 768] [1, 1] : tensor<32000x768xi8> to tensor<768xi8>
	%x_in = func.call @widen_v(%x_row) : (tensor<768xi8>) -> tensor<768xi32>

	%cos = tensor.extract_slice %rope_cos [%pos_idx, 0] [1, 384] [1, 1] : tensor<1024x384xi32> to tensor<384xi32>
	%sin = tensor.extract_slice %rope_sin [%pos_idx, 0] [1, 384] [1, 1] : tensor<1024x384xi32> to tensor<384xi32>

	// ---- decoder layers ----------------------------------------------------
	%xL, %kcL, %vcL = scf.for %layer = %c0 to %c6 step %c1
			iter_args(%x = %x_in, %kc0 = %kc, %vc0 = %vc)
			-> (tensor<768xi32>, tensor<6x1024x768xi8>, tensor<6x1024x768xi8>) {
		%rms_att_w = tensor.extract_slice %rms_att_weights [%layer, 0] [1, 768] [1, 1] : tensor<6x768xi32> to tensor<768xi32>
		%xb = func.call @irmsnorm(%x, %rms_att_w) : (tensor<768xi32>, tensor<768xi32>) -> tensor<768xi8>

		// Q, K, V in one GEMV against the fused weight, requantized together
		%wqkv_l = tensor.extract_slice %wqkv [%layer, 0, 0] [1, 2304, 768] [1, 1, 1] : tensor<6x2304x768xi8> to tensor<2304x768xi8>
		%qkv_acc = cinm.op.gemv %wqkv_l, %xb : tensor<2304x768xi8>, tensor<768xi8> -> tensor<2304xi32>
		%qkv = func.call @requant_qkv(%qkv_acc) : (tensor<2304xi32>) -> tensor<2304xi8>
		%q = tensor.extract_slice %qkv [0] [768] [1] : tensor<2304xi8> to tensor<768xi8>
		%k = tensor.extract_slice %qkv [768] [768] [1] : tensor<2304xi8> to tensor<768xi8>
		%v = tensor.extract_slice %qkv [1536] [768] [1] : tensor<2304xi8> to tensor<768xi8>

		// RoPE on q and k, then the caches take this position's k and v
		%q2 = func.call @irope(%q, %cos, %sin) : (tensor<768xi8>, tensor<384xi32>, tensor<384xi32>) -> tensor<768xi8>
		%k2 = func.call @irope(%k, %cos, %sin) : (tensor<768xi8>, tensor<384xi32>, tensor<384xi32>) -> tensor<768xi8>
		%kc1 = tensor.insert_slice %k2 into %kc0 [%layer, %pos_idx, 0] [1, 1, 768] [1, 1, 1] : tensor<768xi8> into tensor<6x1024x768xi8>
		%vc1 = tensor.insert_slice %v into %vc0 [%layer, %pos_idx, 0] [1, 1, 768] [1, 1, 1] : tensor<768xi8> into tensor<6x1024x768xi8>

		// attention over the whole cache of this layer
		%lkc = tensor.extract_slice %kc1 [%layer, 0, 0] [1, 1024, 768] [1, 1, 1] : tensor<6x1024x768xi8> to tensor<1024x768xi8>
		%lvc = tensor.extract_slice %vc1 [%layer, 0, 0] [1, 1024, 768] [1, 1, 1] : tensor<6x1024x768xi8> to tensor<1024x768xi8>
		%ctx = func.call @imha(%q2, %lkc, %lvc, %attn_mask)
			: (tensor<768xi8>, tensor<1024x768xi8>, tensor<1024x768xi8>, tensor<1024xi32>) -> tensor<768xi8>
		...
		%wo_l = tensor.extract_slice %wo [%layer, 0, 0] [1, 768, 768] [1, 1, 1] : tensor<6x768x768xi8> to tensor<768x768xi8>
		%ao_acc = cinm.op.gemv %wo_l, %ctx : tensor<768x768xi8>, tensor<768xi8> -> tensor<768xi32>
		%ao = func.call @requant_v(%ao_acc) : (tensor<768xi32>) -> tensor<768xi8>
		%ao_w = func.call @widen_v(%ao) : (tensor<768xi8>) -> tensor<768xi32>
		%x1 = cinm.op.elementwise add %x, %ao_w : tensor<768xi32>

		// feed-forward: w2(silu(w1 x) * w3 x), 768 -> 2048 -> 768
		%rms_ffn_w = tensor.extract_slice %rms_ffn_weights [%layer, 0] [1, 768] [1, 1] : tensor<6x768xi32> to tensor<768xi32>
		%xf = func.call @irmsnorm(%x1, %rms_ffn_w) : (tensor<768xi32>, tensor<768xi32>) -> tensor<768xi8>
		%w1_l = tensor.extract_slice %w1 [%layer, 0, 0] [1, 2048, 768] [1, 1, 1] : tensor<6x2048x768xi8> to tensor<2048x768xi8>
		%w3_l = tensor.extract_slice %w3 [%layer, 0, 0] [1, 2048, 768] [1, 1, 1] : tensor<6x2048x768xi8> to tensor<2048x768xi8>
		%w2_l = tensor.extract_slice %w2 [%layer, 0, 0] [1, 768, 2048] [1, 1, 1] : tensor<6x768x2048xi8> to tensor<768x2048xi8>
		%h1_acc = cinm.op.gemv %w1_l, %xf : tensor<2048x768xi8>, tensor<768xi8> -> tensor<2048xi32>
		%h3_acc = cinm.op.gemv %w3_l, %xf : tensor<2048x768xi8>, tensor<768xi8> -> tensor<2048xi32>
		%h1 = func.call @requant_f(%h1_acc) : (tensor<2048xi32>) -> tensor<2048xi8>
		%h3 = func.call @requant_f(%h3_acc) : (tensor<2048xi32>) -> tensor<2048xi8>
		%hb = func.call @iswiglu(%h1, %h3) : (tensor<2048xi8>, tensor<2048xi8>) -> tensor<2048xi8>
		%f_acc = cinm.op.gemv %w2_l, %hb : tensor<768x2048xi8>, tensor<2048xi8> -> tensor<768xi32>
		%f = func.call @requant_v(%f_acc) : (tensor<768xi32>) -> tensor<768xi8>
		%f_w = func.call @widen_v(%f) : (tensor<768xi8>) -> tensor<768xi32>
		%x2 = cinm.op.elementwise add %x1, %f_w : tensor<768xi32>
		...
		%x2m = bufferization.materialize_in_destination %x2 in %x : (tensor<768xi32>, tensor<768xi32>) -> tensor<768xi32>
		scf.yield %x2m, %kc1, %vc1 : tensor<768xi32>, tensor<6x1024x768xi8>, tensor<6x1024x768xi8>
	} {unroll_layers}
	...
	%xn = func.call @irmsnorm(%xL, %rms_final_weight) : (tensor<768xi32>, tensor<768xi32>) -> tensor<768xi8>
	%logits_p = cinm.op.gemv %wcls, %xn : tensor<32768x768xi8>, tensor<768xi8> -> tensor<32768xi32>
...
//
// Per head: scores = K_h q_h / sqrt(48) over all 1024 cache rows (a GEMV
...
// 1 x 1024 by 1024 x 48 product. Heads accumulate into one i32 context and
// are requantized together after the loop.
// ===========================================================================
func.func @imha(%q : tensor<768xi8>, %kc : tensor<1024x768xi8>, %vc : tensor<1024x768xi8>,
                %mask : tensor<1024xi32>) -> tensor<768xi8> {
	%c0 = arith.constant 0 : index
	%c48 = arith.constant 48 : index
	%c768 = arith.constant 768 : index
	%init_ctx = tensor.empty() : tensor<768xi32>

	%ctx_acc = scf.for %h = %c0 to %c768 step %c48 iter_args(%ctx_i = %init_ctx) -> (tensor<768xi32>) {
		%qs = tensor.extract_slice %q [%h] [48] [1] : tensor<768xi8> to tensor<48xi8>
		%kh = tensor.extract_slice %kc [0, %h] [1024, 48] [1, 1] : tensor<1024x768xi8> to tensor<1024x48xi8>
		%vh = tensor.extract_slice %vc [0, %h] [1024, 48] [1, 1] : tensor<1024x768xi8> to tensor<1024x48xi8>

		%sc_acc = cinm.op.gemv %kh, %qs : tensor<1024x48xi8>, tensor<48xi8> -> tensor<1024xi32>
		...
		%ch2d = cinm.op.gemm %p2d, %vh : tensor<1x1024xi8>, tensor<1024x48xi8> -> tensor<1x48xi32>
		%ch = tensor.collapse_shape %ch2d [[0, 1]] : tensor<1x48xi32> into tensor<48xi32>
		%ctx_o = tensor.insert_slice %ch into %ctx_i [%h] [48] [1] : tensor<48xi32> into tensor<768xi32>
		scf.yield %ctx_o : tensor<768xi32>
	} {unroll_heads}

	%ctx = func.call @requant_v(%ctx_acc) : (tensor<768xi32>) -> tensor<768xi8>
	return %ctx : tensor<768xi8>
}
...
// ===========================================================================
func.func @irope(%q : tensor<768xi8>, %cos : tensor<384xi32>, %sin : tensor<384xi32>) -> tensor<768xi8> {
	%q0 = tensor.extract_slice %q [0] [384] [2] : tensor<768xi8> to tensor<384xi8>
	%q1 = tensor.extract_slice %q [1] [384] [2] : tensor<768xi8> to tensor<384xi8>
	%init = tensor.empty() : tensor<384xi8>
	%r0 = linalg.map ins(%q0, %q1, %cos, %sin : tensor<384xi8>, tensor<384xi8>, tensor<384xi32>, tensor<384xi32>) outs(%init : tensor<384xi8>)
	...
	%r1 = linalg.map ins(%q0, %q1, %cos, %sin : tensor<384xi8>, tensor<384xi8>, tensor<384xi32>, tensor<384xi32>) outs(%init : tensor<384xi8>)
		(%a : i8, %b : i8, %c : i32, %s : i32, %o : i8) {
			%q14 = arith.constant 14 : i32
			%a32 = arith.extsi %a : i8 to i32
			%b32 = arith.extsi %b : i8 to i32
	...
	%o0 = tensor.insert_slice %r0 into %q [0] [384] [2] : tensor<384xi8> into tensor<768xi8>
	%o1 = tensor.insert_slice %r1 into %o0 [1] [384] [2] : tensor<384xi8> into tensor<768xi8>
	return %o1 : tensor<768xi8>
}

// ===========================================================================
// I-RMSNorm  (the sqrt of I-BERT 3.4, without the mean)
//
//   ss    = sum((x >> 3)^2)            -- pre-shifted so 768 squares fit i32
...
// ===========================================================================
func.func @irmsnorm(%x : tensor<768xi32>, %w : tensor<768xi32>) -> tensor<768xi8> {
	%init = tensor.empty() : tensor<768xi32>
	%init_o = tensor.empty() : tensor<768xi8>
	%pre = arith.constant 3 : i32
	%h = arith.constant 768 : i32
	...
	%xs = linalg.map ins(%x : tensor<768xi32>) outs(%init : tensor<768xi32>)
		(%v : i32, %o : i32) {
			%r = arith.shrsi %v, %pre : i32
			linalg.yield %r : i32
		}
	%sq = cinm.op.elementwise mul %xs, %xs : tensor<768xi32>
	%ss = cinm.op.reduce add (%sq) : tensor<768xi32> -> i32
	...
	%out = linalg.map ins(%x, %w : tensor<768xi32>, tensor<768xi32>) outs(%init_o : tensor<768xi8>)
		(%v : i32, %g : i32, %o : i8) {
...
// ===========================================================================
func.func @iswiglu(%h1 : tensor<2048xi8>, %h3 : tensor<2048xi8>) -> tensor<2048xi8> {
	%init_o = tensor.empty() : tensor<2048xi8>
	%r = linalg.map ins(%h1, %h3 : tensor<2048xi8>, tensor<2048xi8>) outs(%init_o : tensor<2048xi8>)
		(%a : i8, %g : i8, %o : i8) {
			%ln2 = arith.constant 11 : i32
			%pa = arith.constant 5874 : i32
			%pb = arith.constant 22168 : i32
...
// header describes. The product genuinely needs 64 bits: a 2048-deep int8
// accumulation reaches ~3e7, and M is ~2e4.
...
func.func @requant_qkv(%x : tensor<2304xi32>) -> tensor<2304xi8> {
	%init = tensor.empty() : tensor<2304xi8>
	%r = linalg.map ins(%x : tensor<2304xi32>) outs(%init : tensor<2304xi8>)
		(%v : i32, %o : i8) {
			%y = func.call @requant_scalar(%v) : (i32) -> i8
			linalg.yield %y : i8
		}
	return %r : tensor<2304xi8>
}

func.func @requant_f(%x : tensor<2048xi32>) -> tensor<2048xi8> {
	%init = tensor.empty() : tensor<2048xi8>
	%r = linalg.map ins(%x : tensor<2048xi32>) outs(%init : tensor<2048xi8>)
		(%v : i32, %o : i8) {
			%y = func.call @requant_scalar(%v) : (i32) -> i8
			linalg.yield %y : i8
		}
...
func.func @widen_v(%x : tensor<768xi8>) -> tensor<768xi32> {
	%init = tensor.empty() : tensor<768xi32>
	%r = linalg.map ins(%x : tensor<768xi8>) outs(%init : tensor<768xi32>)
		(%v : i8, %o : i32) {
...
// folds in 1/sqrt(48) ~ 1/8, so it shifts three places further than
