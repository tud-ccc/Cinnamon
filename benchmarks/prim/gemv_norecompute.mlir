...
// (A * x) * c as two cinm ops: the scaling of x, then the gemv. Elementwise
// fusion keeps them apart (fuse-with-recompute=false), so the scaling is a
// distributed op of its own and the search space carries its tiles and a
// fusion edge. gemv.mlir is the same computation as one fused op, the form
// ATiM's schedule takes; measuring both is how the two strategies are
// compared, since no single space holds them.
...
func.func @gemv_norecompute_4MB(%A: tensor<1024x1024xi32> {cinm.static}, %x: tensor<1024xi32>, %c: i32 {cinm.static}) -> tensor<1024xi32>
...
func.func @gemv_norecompute_64MB(%A: tensor<4096x4096xi32> {cinm.static}, %x: tensor<4096xi32>, %c : i32{cinm.static}) -> tensor<4096xi32>
  attributes {cinm.available_platforms = [#upmem]} {
...
func.func @gemv_norecompute_256MB(%A: tensor<8192x8192xi32> {cinm.static}, %x: tensor<8192xi32>, %c : i32{cinm.static}) -> tensor<8192xi32>
  attributes {cinm.available_platforms = [#upmem]} {
...
func.func @gemv_norecompute_512MB(%A: tensor<8192x16384xi32> {cinm.static}, %x: tensor<16384xi32>, %c: i32{cinm.static}) -> tensor<8192xi32>
  attributes {cinm.available_platforms = [#upmem]} {
  %5 = cinm.compute -> tensor<8192xi32> {
