...
// CHECK-NEXT:  %[[BUF0:.*]] = cnm.alloc() for %[[WG]]
// CHECK-NEXT:  %[[BUF1:.*]] = cnm.alloc() for %[[WG]]
// CHECK-NEXT:  %[[BUF2:.*]] = cnm.alloc() for %[[WG]]
// CHECK:       affine.for
// CHECK:         affine.for
// CHECK:           affine.for
  ...
  %result = affine.for %i = 0 to 8 iter_args(%acc0 = %out) -> tensor<8x2048xi32> {
    %r0 = affine.for %j = 0 to 2048 step 128 iter_args(%acc1 = %acc0) -> tensor<8x2048xi32> {
      %r1 = affine.for %k = 0 to 1024 step 256 iter_args(%acc2 = %acc1) -> tensor<8x2048xi32> {
        ...
        %buf0 = cnm.alloc() for %wg : !cnm.buffer<256xi32 on #upmem_2_4_16>
        %buf1 = cnm.alloc() for %wg : !cnm.buffer<256xi32 on #upmem_2_4_16>
        %buf2 = cnm.alloc() for %wg : !cnm.buffer<i32 on #upmem_2_4_16>
...
// CHECK-NEXT:  %[[BUF_A:.*]] = cnm.alloc() for %[[WG0]]
// CHECK:       %[[WG1:.*]] = cnm.workgroup
// CHECK-NEXT:  %[[BUF_B:.*]] = cnm.alloc() for %[[WG1]]
// CHECK:       affine.for
// CHECK-NOT:     = cnm.workgroup
// CHECK-NOT:     cnm.alloc
func.func @hoist_two_independent_wgs(%arg0: tensor<1x256xi32>) -> tensor<1x256xi32> {
  ...
  %result = affine.for %k = 0 to 1024 step 256 iter_args(%acc = %out) -> tensor<1x256xi32> {
    ...
    %buf_a = cnm.alloc() for %wg0 : !cnm.buffer<256xi32 on #upmem_2_4_16>
    %wg1  = cnm.workgroup : !cnm.workgroup<#upmem_2_4_16>
    %buf_b = cnm.alloc() for %wg1 : !cnm.buffer<256xi32 on #upmem_2_4_16>
...
// CHECK:       %[[WG:.*]] = cnm.workgroup
// CHECK-NEXT:  %[[BUF:.*]] = cnm.alloc() for %[[WG]]
