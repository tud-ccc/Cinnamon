...
func.func @matmul(%A: tensor<1024x1024xi32>, %B: tensor<1024x1024xi32>) -> tensor<1024x1024xi32> {
    ...
    ^bb0(%i: index, %j: index):
        ...
        %A_buf = cnm.alloc() for %wg { cnm.physical_space = "global" } : !cnm.buffer<16xi32, level 0> for !cnm.workgroup<4x16>
        ...
        %sc_a_token = cnm.scatter %reshaped into %A_buf[#red_scatter_map] of %wg : tensor<4x16x16xi32> into !cnm.buffer<16xi32, level 0> of !cnm.workgroup<4x16>

        // We alloc a buffer for the reduction result (scalar)
        %outbuf = cnm.alloc() for %wg { cnm.physical_space = "global" } : !cnm.buffer<i32, level 0> for !cnm.workgroup<4x16>
        // Then we launch the group
        %token2 = cnm.launch %wg (%A_buf, %outbuf: !cnm.buffer<16xi32, level 0>, !cnm.buffer<i32, level 0>) on !cnm.workgroup<4x16> {
        ...
        %ReductionStage1, %token3 = cnm.gather %outbuf[#red_scatter_map] of %wg : !cnm.buffer<i32, level 0> of !cnm.workgroup<4x16> into tensor<4x16xi32>
