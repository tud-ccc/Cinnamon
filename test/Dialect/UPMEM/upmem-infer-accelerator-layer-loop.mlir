// RUN: cinm-opt %s --split-input-file --cinm-absorb-static-slices --cinm-isolate-compute-blocks --upmem-infer-accelerator="simulator=op-count max-evals=4 n-init=4 fixed-tasklets=4 graph-allocation=1 allocation-granularity=4 latency-objective=1" | FileCheck %s
