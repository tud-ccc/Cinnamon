...
// RUN:   --upmem-infer-accelerator="simulator=op-count eval-solution=dpus=2048,tasklets=8,gemv.M.mram=8,gemv.K.mram=128,gemv.M.wram=8,gemv.K.wram=64,gemv.order=2" 2>&1 \
...
func.func @gemv_64MB(%A: tensor<4096x4096xi32>, %x: tensor<4096xi32>) -> tensor<4096xi32> {
  ...
  // order 0, the default rule: the k-tile index is outermost, so the eight
  ...
  // order 1: the k-tile index is innermost, so adjacent leaves differ in it and
  ...
  // 4096/128 k-tiles), so the space offers two orders and no more. The space
  // rejects the rest rather than the pass: an index the op has no order for is
  // a configuration the search is never offered, not a trial that fails.
  // RANGE: Configuration is invalid
  // RANGE: violates:
  // RANGE-NEXT: order < (number of dimensions spread over the workgroup)!
