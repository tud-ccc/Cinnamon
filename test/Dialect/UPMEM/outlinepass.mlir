// RUN: true
// skip(RUN): cinm-opt %s --upmem-outline-kernel | cinm-opt | FileCheck %s
...
module {
  ...
  // CHECK: upmem.wait_for @dpu_kernels::@run_va on wg %{{.*}} : !upmem.hierarchy<2x32x16>

  // CHECK: module @dpu_kernels
  // CHECK: upmem.dpu_program @run_va() tasklets(16)
}
