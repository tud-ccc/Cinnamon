...
module {
  ...
  func.func @uniform_blocks() {
    ...
    %1 = upmem.alloc_dpus with program @dpu_kernels::@program : !upmem.hierarchy<1x512x8>
    upmem.scatter_blocks %0[8 elts, affine_map<(d0, d1, d2) -> (0, 0)>, 8 blocks] onto @buf of %1
        : memref<1x8xi32> onto !upmem.hierarchy<1x512x8>
    return
  }
  module @dpu_kernels {
...
// CHECK-DAG: #[[ROW:[^ ]*]] = affine_map<(d0, d1) -> (d1, 0)>
...
module {
  func.func @adjacent_blocks(%arg0: memref<128x32xi32>) {
    ...
    upmem.scatter_blocks %arg0[8 elts, affine_map<(d0, d1, d2) -> (d1, d2 * 8)>, 4 blocks] onto @buf of %1
        : memref<128x32xi32> onto !upmem.hierarchy<1x128x4>
    return
  }
  module @dpu_kernels {
...
module {
  func.func @blocks_with_gaps(%arg0: memref<128x64xi32>) {
    %1 = upmem.alloc_dpus with program @dpu_kernels::@program : !upmem.hierarchy<1x128x4>
    upmem.scatter_blocks %arg0[8 elts, affine_map<(d0, d1, d2) -> (d1, d2 * 16)>, 4 blocks] onto @buf of %1
        : memref<128x64xi32> onto !upmem.hierarchy<1x128x4>
    return
  }
  module @dpu_kernels {
    upmem.dpu_program @program() tasklets(4) {
...
// A map that ignores rank and DPU, over the whole host buffer: every DPU gets
...
module {
  func.func @whole_buffer(%arg0: memref<32xi32>) {
    %1 = upmem.alloc_dpus with program @dpu_kernels::@program : !upmem.hierarchy<1x128x1>
    upmem.scatter_on_array %arg0[32 elts, affine_map<(d0, d1) -> (0)>] onto @buf of %1
        : memref<32xi32> onto !upmem.hierarchy<1x128x1>
    return
  }
  module @dpu_kernels {
...
module {
  func.func @per_dpu_rows(%arg0: memref<128x32xi32>) {
    %1 = upmem.alloc_dpus with program @dpu_kernels::@program : !upmem.hierarchy<1x128x1>
    upmem.scatter_on_array %arg0[32 elts, affine_map<(d0, d1) -> (d1, 0)>] onto @buf of %1
        : memref<128x32xi32> onto !upmem.hierarchy<1x128x1>
    return
  }
  module @dpu_kernels {
    upmem.dpu_program @program() tasklets(1) {
...
// CHECK-DAG: #[[GROW:[^ ]*]] = affine_map<(d0, d1) -> (d1, 0)>
...
module {
  func.func @gather_never_broadcasts(%arg0: memref<128x32xi32>) {
    %1 = upmem.alloc_dpus with program @dpu_kernels::@program : !upmem.hierarchy<1x128x4>
    upmem.gather_blocks %arg0[8 elts, affine_map<(d0, d1, d2) -> (d1, d2 * 8)>, 4 blocks] from @buf of %1
        : memref<128x32xi32> from !upmem.hierarchy<1x128x4>
    return
  }
  module @dpu_kernels {
    upmem.dpu_program @program() tasklets(4) {
